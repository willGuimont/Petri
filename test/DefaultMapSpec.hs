...
spec = do
  ...
  describe "adjustWithKey" $
    do
      ...
      it "does not modify other values" $
        property $
          \(Fn (fUncurried :: (KeyType, ValueType) -> ValueType))
           ( k ::
               KeyType
             )
           m ->
...
applicativeCompositionProp ::
  DM.DefaultMap KeyType (Fun OtherType AnotherType) ->
  DM.DefaultMap KeyType (Fun ValueType OtherType) ->
  DM.DefaultMap KeyType ValueType ->
  IO ()
applicativeCompositionProp uWrapped vWrapped w =
  (pure (.) <*> u <*> v <*> w)
    `shouldBe` (u <*> (v <*> w))
  where
...
applicativeInterchangeProp uWrapped y =
  (u <*> pure y)
    `shouldBe` (pure ($ y) <*> u)
  where
    u = fmap applyFun uWrapped
...
applicativeFunctorConsistentProp (Fn f) m =
  (fmap f m)
    `shouldBe` (pure f <*> m)

foldableProp ::
  Fun (ValueType, OtherType) OtherType ->
  [(KeyType, ValueType)] ->
  OtherType ->
  ValueType ->
  IO ()
foldableProp fWrapped xs z def =
  foldr f z (DM.fromList def xs)
